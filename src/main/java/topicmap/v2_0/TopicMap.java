//
// This file was generated by the JavaTM Architecture for XML Binding(JAXB) Reference Implementation, v2.2.8-b130911.1802 
// See <a href="http://java.sun.com/xml/jaxb">http://java.sun.com/xml/jaxb</a> 
// Any modifications to this file will be lost upon recompilation of the source schema. 
// Generated on: 2017.01.18 at 09:17:02 AM MST 
//

package topicmap.v2_0;

import java.util.ArrayList;
import java.util.List;

import javax.xml.bind.annotation.XmlAccessType;
import javax.xml.bind.annotation.XmlAccessorType;
import javax.xml.bind.annotation.XmlAttribute;
import javax.xml.bind.annotation.XmlElement;
import javax.xml.bind.annotation.XmlElements;
import javax.xml.bind.annotation.XmlRootElement;
import javax.xml.bind.annotation.XmlSchemaType;
import javax.xml.bind.annotation.XmlType;

/**
 * <p>
 * Java class for anonymous complex type.
 * 
 * <p>
 * The following schema fragment specifies the expected content contained within this class.
 * 
 * <pre>
 * &lt;complexType>
 *   &lt;complexContent>
 *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *       &lt;sequence>
 *         &lt;element ref="{http://www.topicmaps.org/xtm/}itemIdentity" maxOccurs="unbounded" minOccurs="0"/>
 *         &lt;element ref="{http://www.topicmaps.org/xtm/}mergeMap" maxOccurs="unbounded" minOccurs="0"/>
 *         &lt;choice maxOccurs="unbounded" minOccurs="0">
 *           &lt;element ref="{http://www.topicmaps.org/xtm/}topic"/>
 *           &lt;element ref="{http://www.topicmaps.org/xtm/}association"/>
 *         &lt;/choice>
 *       &lt;/sequence>
 *       &lt;attribute name="reifier" type="{http://www.w3.org/2001/XMLSchema}anyURI" />
 *       &lt;attribute name="version" type="{http://www.w3.org/2001/XMLSchema}anySimpleType" fixed="2.0" />
 *     &lt;/restriction>
 *   &lt;/complexContent>
 * &lt;/complexType>
 * </pre>
 * 
 * 
 */
@XmlAccessorType(XmlAccessType.FIELD)
@XmlType(name = "", propOrder = {
        "itemIdentity",
        "mergeMap",
        "topicOrAssociation"
})
@XmlRootElement(name = "topicMap")
public class TopicMap {

    protected List<ItemIdentity> itemIdentity;
    protected List<MergeMap> mergeMap;
    @XmlElements({
            @XmlElement(name = "topic", type = Topic.class),
            @XmlElement(name = "association", type = Association.class)
    })
    protected List<Object> topicOrAssociation;
    @XmlAttribute(name = "reifier")
    @XmlSchemaType(name = "anyURI")
    protected String reifier;
    @XmlAttribute(name = "version")
    @XmlSchemaType(name = "anySimpleType")
    protected String version;

    /**
     * Gets the value of the itemIdentity property.
     * 
     * <p>
     * This accessor method returns a reference to the live list,
     * not a snapshot. Therefore any modification you make to the
     * returned list will be present inside the JAXB object.
     * This is why there is not a <CODE>set</CODE> method for the itemIdentity property.
     * 
     * <p>
     * For example, to add a new item, do as follows:
     * 
     * <pre>
     * getItemIdentity().add(newItem);
     * </pre>
     * 
     * 
     * <p>
     * Objects of the following type(s) are allowed in the list
     * {@link ItemIdentity }
     * 
     * 
     */
    public List<ItemIdentity> getItemIdentity() {
        if (itemIdentity == null) {
            itemIdentity = new ArrayList<ItemIdentity>();
        }
        return this.itemIdentity;
    }

    /**
     * Gets the value of the mergeMap property.
     * 
     * <p>
     * This accessor method returns a reference to the live list,
     * not a snapshot. Therefore any modification you make to the
     * returned list will be present inside the JAXB object.
     * This is why there is not a <CODE>set</CODE> method for the mergeMap property.
     * 
     * <p>
     * For example, to add a new item, do as follows:
     * 
     * <pre>
     * getMergeMap().add(newItem);
     * </pre>
     * 
     * 
     * <p>
     * Objects of the following type(s) are allowed in the list
     * {@link MergeMap }
     * 
     * 
     */
    public List<MergeMap> getMergeMap() {
        if (mergeMap == null) {
            mergeMap = new ArrayList<MergeMap>();
        }
        return this.mergeMap;
    }

    /**
     * Gets the value of the topicOrAssociation property.
     * 
     * <p>
     * This accessor method returns a reference to the live list,
     * not a snapshot. Therefore any modification you make to the
     * returned list will be present inside the JAXB object.
     * This is why there is not a <CODE>set</CODE> method for the topicOrAssociation property.
     * 
     * <p>
     * For example, to add a new item, do as follows:
     * 
     * <pre>
     * getTopicOrAssociation().add(newItem);
     * </pre>
     * 
     * 
     * <p>
     * Objects of the following type(s) are allowed in the list
     * {@link Topic }
     * {@link Association }
     * 
     * 
     */
    public List<Object> getTopicOrAssociation() {
        if (topicOrAssociation == null) {
            topicOrAssociation = new ArrayList<Object>();
        }
        return this.topicOrAssociation;
    }

    /**
     * Gets the value of the reifier property.
     * 
     * @return
     *         possible object is
     *         {@link String }
     * 
     */
    public String getReifier() {
        return reifier;
    }

    /**
     * Sets the value of the reifier property.
     * 
     * @param value
     *            allowed object is
     *            {@link String }
     * 
     */
    public void setReifier(String value) {
        this.reifier = value;
    }

    /**
     * Gets the value of the version property.
     * 
     * @return
     *         possible object is
     *         {@link String }
     * 
     */
    public String getVersion() {
        if (version == null) {
            return "2.0";
        } else {
            return version;
        }
    }

    /**
     * Sets the value of the version property.
     * 
     * @param value
     *            allowed object is
     *            {@link String }
     * 
     */
    public void setVersion(String value) {
        this.version = value;
    }

}
